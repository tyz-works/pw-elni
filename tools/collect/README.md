...
- **新日本**: カードのページは名前が平坦に並ぶだけで**陣営の区切りが無い**
  （結果ページのカードも同じ。`parseCard` の `names` も平坦）。陣営は記事本文を
  読んだ LLM が組み立てる仕事だが、開催前には記事が無い。名前の羅列から陣営を
  推測させることになるので取らない
- **DDT**: そもそもカードが載らない（「■出演予定選手」は出場予定の一覧）

**未登録の選手がいるとその興行は書かれない**（`stage` は `unresolved` があると
走らない）。開催前のカードは新人・来日選手を含みやすいので、レポートの
`unresolved` を見て `data/wrestlers/` に足すこと。暫定カードは丸ごと
差し替わるので、足せば翌日そのまま入る。
