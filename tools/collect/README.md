...
開催前のカードは**暫定**として扱う。公式が差し替えたらこちらも差し替え、
興行後は結果ページから全試合を取り直して置き換える。

`merge` は通常 `segment` + `order` で試合を突き合わせるが、**既存の試合が
すべて `result: null` なら matches を丸ごと入れ替える**。開催前のカードには
第N試合の番号が無く、結果ページの番号と一致する保証がないので、突き合わせると
番号のずれた古い試合が残ってしまう。1 つでも結果が入っていれば従来の
突き合わせに戻り、「既存値は上書きしない」がそのまま効く。

カードを取れるのはスターダムと新日本:

- **スターダム**: `≪王者≫ / VS / ≪挑戦者≫` で陣営が分かる。実データでは
  「挑戦者組」だけ `≪≫` が付いておらず括弧の有無が揺れるので、どちらでも拾う
- **新日本**: **陣営は `innerText` には出ない。** 名前が縦に並ぶだけで、どちらの
  チームかが落ちる（結果ページのカードも同じ）。DOM では 2 カラムに分かれて
  いるので、`fetchRaw` が `fetcher.fetchWithDom` でそこだけ構造を読み出し、
  本文の後ろに `=== pw-elni: 対戦カード（DOM から） ===` の節として足す。
  スナップショットから先はこれまでどおりテキストの解析で完結する
- **DDT**: そもそもカードが載らない（「■出演予定選手」は出場予定の一覧）

新日本の DOM 抽出は**クラス名に依存しない**。「第N試合で始まり選手リンクを含む
最も内側の要素」を試合ブロックとし、その中で「選手リンクを持つ子が 2 つ以上あり、
ブロック内の全リンクをちょうど尽くす最も外側の要素」を陣営の分割点にする。
グリッドには空の子要素が混じるので、子の総数では判定できない。

**未登録の選手がいる試合は書かない。** 開催前のカードは新人・来日選手を含み
やすく、1 人未登録なだけで興行ごと落とすと日時・会場まで失われる（カードを
取り始める前より悪くなる）。そこで開催前の興行に限り、解決できなかった試合だけ
落として興行は書く。**公式の試合番号はそのまま残す**ので、抜けた試合は番号の
飛びで分かる。名前はレポートの `unresolved` に出るので `data/wrestlers/` に
足すこと。暫定カードは丸ごと差し替わるので、足せば翌日そのまま入る。

会場が解決できない場合は従来どおり興行ごと書かない（スキーマ上必須のため）。
