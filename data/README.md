...
- **DDT**: 結果ページの冒頭に「★大会ハイライト★」で挟まれた番組表がある。公式がその日の
  進行をそのまま並べたもので、試合以外（前説・ライブ・公開調印式）も含む。アダプタは
  見出しの語彙を列挙せず、この番組表で本文を区切る
- **DDT**: 発表済みだが試合順が未発表の興行は、公式の対戦カード掲載順を**反転して** `order` に振り、
  その旨を先頭の試合の `notes` に書く。DDT の対戦カードはメインイベントを先頭に並べる
  （末尾がダークマッチ）ので、掲載順をそのまま使うとメインが第 1 試合になってしまう。
  ダークマッチは `segment: "dark"` にして本戦の番号から外す
...
## 試合の番号（`order` と `segment`）

`order` は **`segment` ごとの連番**。`segment` は次の 2 つ。

| `segment` | 意味 | 表示 |
|---|---|---|
| `card` | 公式が試合番号を振る本戦 | 第 N 試合 |
| `dark` | ダークマッチ（番号の外・本戦の前に行われる） | ダークマッチ／第 N ダークマッチ |

DDT は「オープニングマッチ＝第 1 試合」で、ダークマッチはその番号の外にある。同じ興行に
`dark:1` と `card:1` が並ぶのが正しい状態で、検証器も `segment` ごとに重複を見る。
ダークマッチが 1 つだけなら UI は番号を付けない（公式もそう書く）。

**番号は「解析できた試合の連番」ではなく「番組表に載った試合の位置」で振る。**
連番にすると、解析に失敗した試合の後ろが 1 つずつ繰り上がって別の試合の番号になる。
取りこぼした試合の番号は欠番のまま残す。

## 同じリングネームの複数名

公式が 1 つの陣営に同じリングネームを複数回並べることがある（偽バラモンユウキ 3 名、
男色ディーノ 2 名）。中の人が違うので、**公式の記載どおりに持つ**。`wrestlerIds` は
陣営内の重複を許す。

ただし黙って通すとこちらの取り違えを見逃すので、収集レポートの「同じ名前が 1 つの陣営に
複数」に上げる。**同じ人物に見えても、公式が別の名前で並べていれば別の選手として扱う**
（「透明・平田一喜」と「平田一喜」は別 slug）。
...
- **試合の整合** — 同じ `segment` の中での `order` の重複、同一選手が両陣営に入っていないか、
